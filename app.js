(() => {
    ...
    let innerRotation   = 0;      // current inner-disc rotation (degrees)
    let selectedOriginIdx  = 0;
    let selectedDestIdx    = 0;
    let dragging = null;          // null | 'outer' | 'inner'
    ...
    function addCountryLabel(parent, name, angleDeg, textR, ring, idx) {
        ...
        // Hit area + click-to-rotate for both rings
    ...
    function setupDrag() {
        ...
        container.addEventListener('pointerdown', (e) => {
            ...
            } else if (dist > CENTER_R && dist <= INNER_R) {
                dragging = 'inner';
                startAngle = ptrAngle(e);
        ...
        container.addEventListener('pointermove', (e) => {
            ...
            if (!dragMoved && Math.abs(delta) > 2) {
                dragMoved = true;
                container.setPointerCapture(pendingPointerId);
            }

            if (dragMoved) {
                if (dragging === 'outer') {
                    outerRotation = startRotation + delta;
                    document.getElementById('outer-disc').setAttribute('transform', `rotate(${outerRotation})`);
                    updateOriginFromRotation(false);
                } else {
                    innerRotation = startRotation + delta;
                    document.getElementById('inner-disc').setAttribute('transform', `rotate(${innerRotation})`);
                    updateDestFromRotation(false);
                }
            }
        });
        ...
        container.addEventListener('pointerup', (e) => {
            ...
            if (dragMoved) {
                ...
                if (which === 'outer') snapToNearest('outer');
                else snapToNearest('inner');
            }
        });
        ...
        // Suppress click on inner labels when drag occurred
    ...
    function snapToNearest(ring) {
        if (ring === 'outer') {
            const snapped = Math.round(outerRotation / STEP) * STEP;
            animateRotation('outer-disc', outerRotation, snapped, 200, () => {
                outerRotation = snapped;
                updateOriginFromRotation(true);
            });
        } else {
            const snapped = Math.round(innerRotation / STEP) * STEP;
            animateRotation('inner-disc', innerRotation, snapped, 200, () => {
                innerRotation = snapped;
                updateDestFromRotation(true);
            });
        }
    }

    function animateRotation(discId, from, to, duration, onDone) {
        ...
        function frame(time) {
            ...
            disc.setAttribute('transform', `rotate(${cur})`);
            if (t < 1) {
    ...
    function updateDestFromRotation(snap) {
        const norm = ((-innerRotation % 360) + 360) % 360;
        const idx = Math.round(norm / STEP) % N;
        selectDestination(idx);
    }

    /* ── Click-to-rotate ─────────────────────────────────────── */
    function rotateToCountry(ring, idx) {
        const targetRot = -idx * STEP;
        if (ring === 'outer') {
            const diff = targetRot - outerRotation;
            const normDiff = ((diff % 360) + 540) % 360 - 180;
            const target = outerRotation + normDiff;
            animateRotation('outer-disc', outerRotation, target, 400, () => {
                outerRotation = target;
                selectOrigin(idx);
            });
        } else {
            const diff = targetRot - innerRotation;
            const normDiff = ((diff % 360) + 540) % 360 - 180;
            const target = innerRotation + normDiff;
            animateRotation('inner-disc', innerRotation, target, 400, () => {
                innerRotation = target;
                selectDestination(idx);
            });
        }
    }
    ...
    function buildDropdowns() {
        ...
        destSel.addEventListener('change', () => {
            ...
            const targetRot = -idx * STEP;
            let target = targetRot;
            const diff = target - innerRotation;
            const normDiff = ((diff % 360) + 540) % 360 - 180;
            target = innerRotation + normDiff;
